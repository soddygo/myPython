...
class HttpService:
    ...
    def url_request(self, url, values={}, method='GET'):
        ...
        if method == 'GET':
            ...
            data22 = urllib.urlencode(param)  # str

            print("data22:" + data22)

            # {
            #     'warnJson': "{'status': '1', 'source': 'ZX', 'hostname': 'Zabbix server', 'severity': 'Disaster', "
            #                 "'title': 'PROBLEM', 'ip': '127.0.0.1', 'origin_id': '255', 'first_occurrence': '2017-06-07 17:02:12', 'summary': 'Too many processes on Zabbix server'}"}


            try:
                # req = request.Request(url, data=data22, headers=headers)
                res = urllib2.urlopen(url, data22)
...
def main():
    try:
        ...
        log_file = open("/usr/local/nagios/mypython/weixin/message.log", "w")
